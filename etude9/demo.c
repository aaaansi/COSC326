...
typedef enum quit_flag_e
{
    RUN,
    QUIT
} quit_flag;
typedef enum attribute_e
{
    FIRST_NAME,
    LAST_NAME,
    PHONE_NUMBER,
...
static int binsearch(attribute attr, char *key, client *clients_list, int l, int r)
{
    ...
    if (r >= l)
    {
        ...
        if (strncasecmp(get_attribute(clients_list[m], attr), key, strlen(key)) == 0)
            return m;
        if (strncasecmp(get_attribute(clients_list[m], attr), key, strlen(key)) > 0)
            return binsearch(attr, key, clients_list, l, m - 1);
...
int main(int argc, char **argv)
{
    ...
    while (NULL != fgets(client_str, MAX_CLIENT_LENGTH, clients_fp))
        clients_size++;
